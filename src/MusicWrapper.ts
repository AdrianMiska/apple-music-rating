...
class Music {
    ...
    async getAuthorizations(): Promise<MusicProvider[]> {
        ...
        if (Object.keys(localStorage).filter(key => key.match(/^music\..*\.media-user-token$/)).length > 0) {
    ...
    async generateSpotifyToken(code: string, state: string) {
        if (state !== localStorage.getItem("spotify_pkce_state")) {
        ...
        queryString.append('client_id', process.env.NEXT_PUBLIC_SPOTIFY_CLIENT_ID || '');
        queryString.append('code_verifier', localStorage.getItem("spotify_pkce_code_verifier") || '');

        let {access_token, expires_in, refresh_token} = await fetch(`https://accounts.spotify.com/api/token`, {
            method: 'POST',
            headers: {
    ...
    private async saveSpotifyToken(access_token: string, expires_in: number, refresh_token: string) {

        localStorage.setItem("spotify_access_token", access_token);
        localStorage.setItem("spotify_expires", String(Math.floor(Date.now() / 1000) + expires_in));
    ...
    private async authorizeSpotify() {
        ...
        let pkceState = window.btoa(window.crypto.getRandomValues(new Uint8Array(16)).toString());
        let {code_challenge, code_verifier} = pkceChallenge();

        localStorage.setItem("spotify_pkce_state", pkceState);
        localStorage.setItem("spotify_pkce_code_verifier", code_verifier);
    ...
    async getPlaylist(playlistId: string | 'apple-music-favorites' | 'spotify-saved-tracks'): Promise<Playlist | undefined> {
        ...
        if (playlistId.match(/^pl?\..+$/)) {
            ...
            return new Playlist(playlist, tracks.items.map(track => new Song(track.track)));
        }
    ...
    async searchPlaylist(searchTerm: string, onlyApple: boolean = false): Promise<Playlist[]> {
        ...
        if (musicProviders.includes(MusicProvider.AppleMusic)) {
            ...
            let musicKitResults = results.map((playlist: MusicKit.Playlists | MusicKit.LibraryPlaylists) => new Playlist(playlist));
        ...
        if (musicProviders.includes(MusicProvider.Spotify) && !onlyApple) {
            ...
            let spotifyResults = response.playlists.items.filter(playlist => playlist.owner.id === currentUserId).map(playlist => new Playlist(playlist));
            res.push(...spotifyResults);
        }
        return res;
    }

    async createPlaylist(name: string, description: string, musicProvider: MusicProvider): Promise<Playlist | null> {
     ...
     * @param songs
     */
    private async filterAppleMusicFavorites(songs: MusicKit.Songs[]): Promise<MusicKit.Songs[]> {
        ...
        let favoriteIds: string[] = (await Promise.all(chunks.map(async (chunk) => {
            // @ts-ignore
    ...
    async getAppleMusicFavorites(): Promise<Song[]> {
        ...
        // @ts-ignore
        let {data: {data: songs, meta: {total}}} = (await musicKit.api.music("v1/me/library/songs", {
            limit: limit,
            offset: offset
        }));
        ...
        for (let i = 1; i < Math.ceil(total / limit); i++) {
            // @ts-ignore
            promises.push(musicKit.api.music("v1/me/library/songs", {
                limit: limit,
...
export class Playlist {
    ...
    constructor(playlist: MusicKit.Playlists | MusicKit.LibraryPlaylists | SpotifyApi.PlaylistObjectSimplified, songs?: Song[]) {
