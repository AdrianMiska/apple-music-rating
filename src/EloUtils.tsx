...
export async function calculateElo(
  ...
  const newBaselineRating =
    baselineRecord.rating + (scoreBaseline - expectedBaseline) * 32;
  const newCandidateRating =
    candidateRecord.rating + (scoreCandidate - expectedCandidate) * 32;
  await setEloRating(
    playlistId,
    baseline,
    newBaselineRating,
